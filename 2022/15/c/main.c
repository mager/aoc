...
int main(int argc, char *argv[])
{
    ...
    for (int64_t x = grid.min.x - (dy / 2); x <= grid.max.x + (dy / 2); x++)
    {
        ...
        for (size_t s = 0U; s < sc; s++)
        {
            ...
            else if (manhattan_distance(sensors[s].self.x, sensors[s].self.y, x, ROW_CONTEST) <= manhattan_distance(sensors[s].self.x, sensors[s].self.y, sensors[s].beacon.x, sensors[s].beacon.y))
            {
                has_beacon = NO;
                break;
            }
